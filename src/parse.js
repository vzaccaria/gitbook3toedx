let {$fs, $yaml, _} = require('zaccaria-cli');
...
function fixCourseImage(dir, config) {
    let ci = config.course.course_image;
    config.course.real_course_image = path.resolve(`${dir}/${ci}`);
    config.course.course_image = path.basename(config.course.real_course_image);
    info(`Course image: ${config.course.course_image} (${config.course.real_course_image})`);
    return config;
}
...
function parse(dir, config) {
    ...
    return $b.all(files).then(([config]) => {
        ...
        .then((config) => fixCourseImage(dir, config))
